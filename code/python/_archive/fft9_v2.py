#!/usr/bin/env python3
"""
FFT9 Algorithm - Corrected Implementation

Key fix: The paper defines the equation as Laplacian(u) = f (not -Laplacian(u) = f)
The 9-point stencil L_h approximates Laplacian(u), with center coefficient -20/(6h^2) < 0.

For the standard form -Laplacian(u) = f:
  Equation becomes: -L_h u = R_h f, i.e., L_h u = -R_h f

In frequency domain:
  u_hat(k,l) = -lambda_B(k,l) / lambda_A(k,l) * f_hat(k,l)

Date: 2026-04-25
"""

import numpy as np
from scipy.fft import dst, idst
import time


def fft9_solve(n, f_func, bc_func, sx=1.0, sy=1.0):
    """
    FFT9 solver for Poisson equation -u_xx - u_yy = f
    with Dirichlet boundary condition u = g on boundary.
    
    Uses sixth-order 9-point finite difference scheme.
    
    The 9-point stencil:
    L_h = (1/(6h^2)) * [1  4  1; 4 -20 4; 1  4  1]   (approximates Laplacian)
    R_h = (1/360) * [0 48 0; 48  0 48; 0 48 0]         (approximates identity)
    
    For -Laplacian(u) = f:
    -L_h u = R_h f  =>  L_h u = -R_h f
    
    Eigenvalues:
    lambda_A(k,l) = (1/(6h^2)) * [-20 + 8*cos(k*theta) + 8*cos(l*phi) + 4*cos(k*theta)*cos(l*phi)]
    lambda_B(k,l) = (4/15) * [cos(k*theta) + cos(l*phi)]
    
    Solve: u_hat = -lambda_B / lambda_A * f_hat
    """
    # Grid
    x = np.linspace(0, sx, n)
    y = np.linspace(0, sy, n)
    X, Y = np.meshgrid(x, y, indexing='ij')
    hx = sx / (n - 1)
    hy = sy / (n - 1)
    
    # Square grid required for 6th-order scheme
    h = hx
    assert abs(hx - hy) < 1e-12, "FFT9 requires hx = hy"
    
    N = n - 2  # interior points per direction
    
    # Step 1: Evaluate RHS at ALL grid points (including boundary)
    F = f_func(X, Y)
    
    # Step 2: Apply R_h template to f in physical space
    # R_h f_{i,j} = (1/360) * 48 * (f_{i-1,j} + f_{i+1,j} + f_{i,j-1} + f_{i,j+1})
    # For -Laplacian(u) = f, we need: L_h u = -R_h f
    # So RF = -R_h f
    RF = np.zeros((N, N))
    for i in range(N):
        for j in range(N):
            ii = i + 1  # full grid index
            jj = j + 1
            # R_h f at interior point (ii, jj)
            val = F[ii-1, jj] + F[ii+1, jj] + F[ii, jj-1] + F[ii, jj+1]
            RF[i, j] = -(1.0 / 360.0) * 48.0 * val  # negative for -Laplacian(u) = f
    
    # Step 3: Handle non-zero boundary conditions
    # L_h u at boundary-adjacent points involves BC values
    # These must be moved to the RHS
    
    # Evaluate BC
    bc_left = np.broadcast_to(bc_func(x[0], y), y.shape).astype(float).copy()
    bc_right = np.broadcast_to(bc_func(x[-1], y), y.shape).astype(float).copy()
    bc_bottom = np.broadcast_to(bc_func(x, y[0]), x.shape).astype(float).copy()
    bc_top = np.broadcast_to(bc_func(x, y[-1]), x.shape).astype(float).copy()
    
    # BC correction: subtract L_h contribution of BC points from RHS
    # L_h u_{i,j} includes terms from boundary neighbors
    # We need to move these to the RHS: RF = RF - L_h_bc_contribution
    # Since L_h u = RF, and u at boundary is known,
    # the correction is to subtract L_h's contribution from boundary values
    
    bc_corr = np.zeros((N, N))
    
    # Left boundary (x=0, i=0 in full grid, affects i=0 in interior)
    for j in range(N):
        jj = j + 1
        # u_{0,jj} affects L_h u_{1,jj}:
        # From template: 4*u_{0,jj} (left neighbor) + 1*u_{0,jj-1} + 1*u_{0,jj+1} (diagonal)
        val = 4.0 * bc_left[jj]
        if jj - 1 >= 0:
            val += 1.0 * bc_left[jj-1]
        if jj + 1 < n:
            val += 1.0 * bc_left[jj+1]
        bc_corr[0, j] -= (1.0 / (6.0 * h**2)) * val
    
    # Right boundary (x=sx, i=n-1 in full grid)
    for j in range(N):
        jj = j + 1
        val = 4.0 * bc_right[jj]
        if jj - 1 >= 0:
            val += 1.0 * bc_right[jj-1]
        if jj + 1 < n:
            val += 1.0 * bc_right[jj+1]
        bc_corr[N-1, j] -= (1.0 / (6.0 * h**2)) * val
    
    # Bottom boundary (y=0, j=0 in full grid)
    for i in range(N):
        ii = i + 1
        val = 4.0 * bc_bottom[ii]
        if ii - 1 >= 0:
            val += 1.0 * bc_bottom[ii-1]
        if ii + 1 < n:
            val += 1.0 * bc_bottom[ii+1]
        bc_corr[i, 0] -= (1.0 / (6.0 * h**2)) * val
    
    # Top boundary (y=sy, j=n-1 in full grid)
    for i in range(N):
        ii = i + 1
        val = 4.0 * bc_top[ii]
        if ii - 1 >= 0:
            val += 1.0 * bc_bottom[ii-1]
        if ii + 1 < n:
            val += 1.0 * bc_top[ii+1]
        bc_corr[i, N-1] -= (1.0 / (6.0 * h**2)) * val
    
    # Corner corrections (diagonal terms from corners)
    # Bottom-left: u_{0,0} affects L_h u_{1,1} via diagonal term (1/(6h^2))*1*u_{0,0}
    if n > 2:
        bc_corr[0, 0] -= (1.0 / (6.0 * h**2)) * 1.0 * bc_left[0]   # already included? need to check
        # Actually, bc_left[0] = u(0,0) and bc_bottom[0] = u(0,0), so we might double count
    
    RF += bc_corr
    
    # Step 4: 2D Sine Transform
    RF_hat = np.zeros((N, N))
    for i in range(N):
        RF_hat[i, :] = dst(RF[i, :], type=1, norm='ortho')
    for j in range(N):
        RF_hat[:, j] = dst(RF_hat[:, j], type=1, norm='ortho')
    
    # Step 5: Solve in frequency domain
    k_vals = np.arange(1, N + 1)
    cos_k = np.cos(k_vals * np.pi / (N + 1))
    cos_l = np.cos(k_vals * np.pi / (N + 1))  # same since square grid
    
    U_hat = np.zeros((N, N))
    for ki in range(N):
        for li in range(N):
            lambda_A = (1.0 / (6.0 * h**2)) * (
                -20.0 + 8.0 * cos_k[ki] + 8.0 * cos_l[li] + 4.0 * cos_k[ki] * cos_l[li]
            )
            if abs(lambda_A) > 1e-14:
                U_hat[ki, li] = RF_hat[ki, li] / lambda_A
    
    # Step 6: Inverse 2D Sine Transform
    U_int = np.zeros((N, N))
    for ki in range(N):
        U_int[ki, :] = idst(U_hat[ki, :], type=1, norm='ortho')
    for li in range(N):
        U_int[:, li] = idst(U_int[:, li], type=1, norm='ortho')
    
    # Step 7: Assemble full solution
    U = np.zeros((n, n))
    U[0, :] = bc_left
    U[-1, :] = bc_right
    U[:, 0] = bc_bottom
    U[:, -1] = bc_top
    U[1:-1, 1:-1] = U_int
    
    return U


def poisson_5point(n, f_func, bc_func, sx=1.0, sy=1.0):
    """Standard 5-point FD + FFT solver for -Laplacian(u) = f (2nd order)"""
    x = np.linspace(0, sx, n)
    y = np.linspace(0, sy, n)
    X, Y = np.meshgrid(x, y, indexing='ij')
    hx = sx / (n - 1)
    hy = sy / (n - 1)
    N = n - 2
    
    F = f_func(X, Y)
    
    bc_left = np.broadcast_to(bc_func(x[0], y), y.shape).astype(float).copy()
    bc_right = np.broadcast_to(bc_func(x[-1], y), y.shape).astype(float).copy()
    bc_bottom = np.broadcast_to(bc_func(x, y[0]), x.shape).astype(float).copy()
    bc_top = np.broadcast_to(bc_func(x, y[-1]), x.shape).astype(float).copy()
    
    F_adj = F[1:-1, 1:-1].copy()
    F_adj[0, :] -= bc_left[1:-1] / hx**2
    F_adj[-1, :] -= bc_right[1:-1] / hx**2
    F_adj[:, 0] -= bc_bottom[1:-1] / hy**2
    F_adj[:, -1] -= bc_top[1:-1] / hy**2
    
    F_hat = np.zeros((N, N))
    for i in range(N):
        F_hat[i, :] = dst(F_adj[i, :], type=1, norm='ortho')
    for j in range(N):
        F_hat[:, j] = dst(F_hat[:, j], type=1, norm='ortho')
    
    k_vals = np.arange(1, N + 1)
    cos_k = np.cos(k_vals * np.pi / (N + 1))
    
    U_hat = np.zeros((N, N))
    for ki in range(N):
        for li in range(N):
            lam = (2.0/hx**2)*(cos_k[ki]-1.0) + (2.0/hy**2)*(cos_k[li]-1.0)
            if abs(lam) > 1e-14:
                U_hat[ki, li] = -F_hat[ki, li] / lam
    
    U_int = np.zeros((N, N))
    for ki in range(N):
        U_int[ki, :] = idst(U_hat[ki, :], type=1, norm='ortho')
    for li in range(N):
        U_int[:, li] = idst(U_int[:, li], type=1, norm='ortho')
    
    U = np.zeros((n, n))
    U[0, :] = bc_left
    U[-1, :] = bc_right
    U[:, 0] = bc_bottom
    U[:, -1] = bc_top
    U[1:-1, 1:-1] = U_int
    
    return U


def test_convergence():
    """Test FFT9 convergence with zero BC: u = sin(pi*x)*sin(pi*y)"""
    print("=" * 70)
    print("FFT9 Convergence Test (zero BC)")
    print("=" * 70)
    
    def u_exact(x, y):
        return np.sin(np.pi * x) * np.sin(np.pi * y)
    
    # For -Laplacian(u) = f:
    # -u_xx - u_yy = 2*pi^2 * sin(pi*x)*sin(pi*y)
    def f_rhs(x, y):
        return 2.0 * np.pi**2 * np.sin(np.pi * x) * np.sin(np.pi * y)
    
    def bc(x, y):
        return 0.0
    
    ns = [5, 9, 17, 33, 65]
    
    print("\n--- 5-point (2nd order) ---")
    errors_5 = []
    for n in ns:
        U = poisson_5point(n, f_rhs, bc)
        x = np.linspace(0, 1, n)
        y = np.linspace(0, 1, n)
        X, Y = np.meshgrid(x, y, indexing='ij')
        err = np.max(np.abs(U - u_exact(X, Y)))
        errors_5.append(err)
        print(f"  n={n:3d}: error = {err:.6e}")
    
    print("\n--- FFT9 9-point (6th order) ---")
    errors_9 = []
    for n in ns:
        U = fft9_solve(n, f_rhs, bc)
        x = np.linspace(0, 1, n)
        y = np.linspace(0, 1, n)
        X, Y = np.meshgrid(x, y, indexing='ij')
        err = np.max(np.abs(U - u_exact(X, Y)))
        errors_9.append(err)
        print(f"  n={n:3d}: error = {err:.6e}")
    
    # Convergence rates
    print("\n" + "=" * 70)
    print("Convergence Rates")
    print("=" * 70)
    print(f"  {'n':>3s}  {'5pt error':>12s}  {'rate':>5s}  {'9pt error':>12s}  {'rate':>5s}")
    print("  " + "-" * 50)
    for i in range(len(ns)):
        if i == 0:
            print(f"  {ns[i]:3d}  {errors_5[i]:12.6e}   ---  {errors_9[i]:12.6e}   ---")
        else:
            r5 = np.log(errors_5[i-1]/errors_5[i]) / np.log(ns[i]/ns[i-1])
            r9 = np.log(errors_9[i-1]/errors_9[i]) / np.log(ns[i]/ns[i-1])
            print(f"  {ns[i]:3d}  {errors_5[i]:12.6e}  {r5:5.2f}  {errors_9[i]:12.6e}  {r9:5.2f}")


def test_paper_problem():
    """Test against paper Table I: u = 3*x*y*(x-x^2)*(y-y^2)"""
    print("\n" + "=" * 70)
    print("Paper Table I Comparison")
    print("Exact: u = 3*x*y*(x-x^2)*(y-y^2)")
    print("=" * 70)
    
    def u_exact(x, y):
        return 3.0 * x * y * (x - x**2) * (y - y**2)
    
    # -u_xx - u_yy = f
    # u = 3*x^2*(1-x)*y^2*(1-y)
    # u_x = 3*(2x-3x^2)*y^2*(1-y)
    # u_xx = 3*(2-6x)*y^2*(1-y)
    # u_y = 3*x^2*(1-x)*(2y-3y^2)
    # u_yy = 3*x^2*(1-x)*(2-6y)
    # -u_xx - u_yy = -3*(2-6x)*y^2*(1-y) - 3*x^2*(1-x)*(2-6y)
    
    def f_rhs(x, y):
        return -3.0 * (2.0 - 6.0*x) * y**2 * (1.0 - y) - 3.0 * x**2 * (1.0 - x) * (2.0 - 6.0*y)
    
    def bc(x, y):
        return u_exact(x, y)
    
    ns = [5, 9, 17, 33, 65]
    
    print("\n--- FFT9 9-point (6th order) ---")
    errors = []
    for n in ns:
        U = fft9_solve(n, f_rhs, bc)
        x = np.linspace(0, 1, n)
        y = np.linspace(0, 1, n)
        X, Y = np.meshgrid(x, y, indexing='ij')
        err = np.max(np.abs(U - u_exact(X, Y)))
        errors.append(err)
        print(f"  n={n:3d}: error = {err:.6e}")
    
    print("\nPaper results (6th order):")
    print("  N=4:  1.34E-07")
    print("  N=8:  2.10E-09")
    print("  N=16: 3.30E-11")
    print("  N=32: 9.49E-13")
    
    print("\nConvergence rates:")
    for i in range(1, len(ns)):
        rate = np.log(errors[i-1]/errors[i]) / np.log(ns[i]/ns[i-1])
        print(f"  n={ns[i]:3d}: rate = {rate:.2f}")


if __name__ == "__main__":
    test_convergence()
    test_paper_problem()
