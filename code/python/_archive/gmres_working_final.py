"""
GMRES (Generalized Minimal Residual) - Final Working Implementation
====================================================================

Based on: Saad & Schultz (1986) original paper
This implementation has been tested and verified.
"""
...
def gmres(A, b, x0=None, tol=1e-6, max_iter=100, restart=10, return_history=False):
    ...
    while total_iter < max_iter:
        ...
        for j in range(m):
            ...
            if H[j, j] == 0 and H[j + 1, j] == 0:
                c[j] = 1.0
                s[j] = 0.0
            else:
                # Compute rotation to eliminate H[j+1, j]
                denom = np.sqrt(H[j, j]**2 + H[j + 1, j]**2)
                c[j] = H[j, j] / denom
                s[j] = H[j + 1, j] / denom
            
            # Apply rotation to H
            temp_j = c[j] * H[j, j] + s[j] * H[j + 1, j]
