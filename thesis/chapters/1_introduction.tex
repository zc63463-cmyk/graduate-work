...
\begin{itemize}[itemsep=2pt]
  ...
  \item $\sigma = -\kappa^2$：true Helmholtz 方程（时谐波动方程的频域形式），
        矩阵可能不定，当特征值 $\lambda_{p,q}=\kappa^2$ 时出现共振。
\end{itemize}
Poisson 方程与 modified Helmholtz 方程的离散矩阵均为对称正定，
适用 CG 等基于正定性的迭代法；true Helmholtz 方程的离散矩阵在穿过离散谱时可能不定，
...
因此，在统一差分框架下实现 FFT 直接法，并以 GMRES 作为迭代法参照来观察谱性质对收敛行为的影响，
对于理解规则网格上不同求解策略的适用范围具有重要意义。
...
并提出了经典 FACR 混合算法，其理论最优复杂度可达 $O(N^2 \log\log N)$。
...
离散矩阵在穿过离散谱时可能不定，当某特征值 $\lambda_{p,q}\approx\kappa^2$ 时频域分母接近零，
...
\begin{enumerate}[itemsep=2pt]
  \item 在统一的 $(-\nabla^2+\sigma)u=f$ 框架下，系统实现了 FA、CR、FACR-like 与五点基准等 FFT 直接求解器，
        覆盖 Dirichlet、Neumann 和混合边界条件；同时实现并验证了针对 Dirichlet 边界的 FFT9 四阶紧致差分求解器，
        并明确区分 modified Helmholtz（$\sigma=+\kappa^2$，正定）与 true Helmholtz（$\sigma=-\kappa^2$，可能不定且存在近共振风险）；
  \item 对 Neumann 边界条件，采用 ghost-point 方法配合对称化 DCT-I 变换，解决了非对称 Laplacian 矩阵的对角化问题；
  \item 在同一 Dirichlet manufactured solution 上构造精度--成本对比实验，
        比较五点 FFT 直接法、Dirichlet FFT9 和无预处理 GMRES(30) 的误差--时间关系；
        该对比用于展示当前实现下的整体基准趋势，不作为严格 kernel-to-kernel benchmark，
        也不代表所有预处理 Krylov 方法；
  \item 实现了基于 Givens 旋转的 GMRES 迭代法，构造了 2D Helmholtz 稀疏矩阵，
        通过 Gaussian 型非特征右端项和近共振扫描分析 modified/true Helmholtz 方程下
        频域分母和无预处理 Krylov 行为的本质差异；
        其中频域分母热图、Dirichlet 五点条件数校验和离散模态投影实验共同形成
        “离散谱分析--实现校验--数值实验”的证据链；
  \item 通过 Fourier 特征值分析证明了三参数九点修正族在统一常数参数和
        local Fourier symbol 意义下无法提升到六阶，
        为当前 FFT9 四阶格式选择提供了局部符号层面的依据，
        并利用 Lean 4 对反证中的代数核心进行了辅助形式化校验。
\end{enumerate}
...
第\ref{sec:experiments}章给出五组核心数值实验：
Dirichlet 收敛、Neumann/mixed 边界处理、精度--成本对比、
modified/true Helmholtz 谱结构与 GMRES 行为、true Helmholtz 近共振模态放大；
