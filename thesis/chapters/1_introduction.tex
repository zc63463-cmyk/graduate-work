...
其中 Poisson 方程 $-\nabla^2 u = f$ 和 Helmholtz 方程
$(-\nabla^2 + k^2)u = f$ 是两类最基本、最典型的椭圆型方程。
Poisson 方程描述了稳态热传导、静电势等物理过程，
而 Helmholtz 方程则是时间简谐波问题的频域形式，
在声学散射、地震波传播和量子力学中有重要应用~\cite{ShaoWenting2017, LiuJiayuan2019, LiSiqing2020}。
...
循环约化（CR）和 FACR 混合算法更可将复杂度降至 $O(N^2 \log\log N)$。
...
Helmholtz 方程的数值求解面临两个主要挑战：
大波数 $k$ 导致离散矩阵条件数急剧增大，
以及共振波数下的解不唯一性。
Bayliss 等 \cite{Bayliss1983} 研究了迭代法求解 Helmholtz 方程的困难，
Ernst 和 Gander \cite{Ernst2011} 系统分析了经典迭代法求解 Helmholtz
方程的收敛性障碍。
Ihlenburg \cite{Ihlenburg1998} 对有限元方法求解 Helmholtz 方程
进行了系统的误差分析。
...
\begin{enumerate}[itemsep=2pt]
  \item 系统实现了基于 FFT 的五种快速直接求解器（FA、CR、FACR、FFT9、5点基准），
        支持 Dirichlet、Neumann 和混合边界条件；
  \item 对 Neumann 边界条件，采用 ghost-point 方法配合对称化 DCT-I 变换，
        解决了非对称 Laplacian 矩阵的对角化问题；
  \item 实现了基于 Givens 旋转的 GMRES 迭代法，构造了 2D Helmholtz 稀疏矩阵，
        与 FFT 直接法进行了系统对比；
  \item 通过特征值分析证明了六阶紧致差分格式不存在常数修正因子，
        为四阶格式是最优选择提供了理论依据。
\end{enumerate}
