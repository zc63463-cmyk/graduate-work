...
\begin{corollary}[条件数估计]
  ...
  条件数趋于无穷，即共振现象。此时矩阵可能接近奇异或不定，不能按 SPD 条件数解释。
\end{corollary}
...
\begin{theorem}[四阶精度]
  ...
  \end{equation}
  该 $h^4$ 项系数不恒为零
  （例如取 $\xi = \eta \neq 0$ 时 $3\xi^4-8\xi^2\eta^2+3\eta^4 = -2\xi^4 \neq 0$），
  因此误差一般为 $O(h^4)$，四阶是该三参数九点修正族在统一参数约束下的自然精度上限。
...
\begin{theorem}[三参数九点修正族的六阶不可行性\cite{Sutmann2007}]
  ...
  \end{equation}
  其中 $\hat{\lambda}_5 = \frac{2}{h^2}(2-\cos\alpha-\cos\beta)$ 为五点正 Laplacian（$-\Delta$）特征值。
  ...
  其中 $\alpha=-\gamma$ 约束和最终 $c_4$ 矛盾由 Lean 4 辅助验证（见附录~\ref{app:lean4}），
  $\beta=\gamma$ 条件由上式常数项直接给出。
  ...
  因此，对形如~\eqref{eq:modified_compact} 的三参数九点修正族而言，四阶是该族在统一常数参数约束下的自然精度上限。
