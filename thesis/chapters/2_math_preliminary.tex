...
\subsubsection{四阶精度的 Fourier 分析}
...
\begin{theorem}[三参数九点修正族的六阶不可行性\cite{Sutmann2007}]
  ...
  再由 $c_4$ 的模态依赖性得出矛盾。
  具体地，对 $\sigma \neq 0$，代入 $\alpha=-\gamma$, $\beta=\gamma$ 后，
  取两个 Fourier 模式 $(\xi,\eta)=(1,0)$ 与 $(1,1)$，
  $c_4=0$ 给出两个互不相容的 $\gamma$ 条件，
  因此不存在统一常数使所有模式 $c_4$ 同时为零。
  该代数矛盾由 Lean 4 定理 \texttt{helmholtz\_no\_sixth\_order\_real} 辅助验证。
...
\end{equation}
其中边界值在差分格式中以 $-g/h^2$ 出现在左端，移至右端后取加号。
