...
\begin{equation}
  \hat{u}_{p,q} = \frac{\hat{\lambda}_R(p,q)}{-\hat{\lambda}_L(p,q) + \sigma\hat{\lambda}_R(p,q)} \hat{f}_{p,q}
  = \frac{\hat{f}_{p,q}}{-\hat{\lambda}_L(p,q)/\hat{\lambda}_R(p,q) + \sigma}
  \label{eq:fft9_freq}
\end{equation}
...
\end{equation}
由定理~\ref{thm:4th_order}，$D_{p,q}^{(9)} = (\xi_p^2+\eta_q^2+\sigma) + O(h^4)$，
因此频域求解的精度为 $O(h^4)$。

\begin{remark}
  对于 modified Helmholtz（$\sigma=+\kappa^2$），
  因 $-\hat{\lambda}_L > 0$ 且 $\hat{\lambda}_R > 0$，频域除数 $D_{p,q}^{(9)} > 0$ 恒成立，
  矩阵正定，求解无条件稳定。
  对于 true Helmholtz（$\sigma=-\kappa^2$），
  当 $-\hat{\lambda}_L(p,q)/\hat{\lambda}_R(p,q) \approx \kappa^2$ 时分母接近零，
  出现近共振现象。
\end{remark}
...
\begin{remark}[物理空间中的边界修正计算]
  ...
  修正后的等效右端项为：
  \begin{equation}
    \tilde{g}_{i,1} = R_h f_{i,1} + \frac{\sigma}{12}g_D(x_i, 0)
    - \frac{1}{6h^2}\left[g_D(x_{i-1},0)+g_D(x_{i+1},0)+4g_D(x_i,0)\right]
    \label{eq:bnd_correction}
  \end{equation}
  其中第一项来自 $\sigma R_h$ 中边界值的贡献（加号，因为边界值从左端移至右端），
  第二项来自 $L_h$（$L_h \approx \Delta$）中边界值产生的等效源项（负号，因为 $-L_h$ 作用后边界贡献取反）。
...
\STATE 计算频域除数：$D_{p,q} = -\hat{\lambda}_L(p,q)/\hat{\lambda}_R(p,q) + \sigma$
